<?php
/**
 * PropertyVenues Controller
 * 
 * @package 
 * @author  
 */
class PropertyVenuesController
{
		...
		switch ($subRequest) {
			...
			default:
				if (self::$app_controller->check_if_logged($email)) {
					...
					$current 			= 'property_venues' 	 . self::$property_id;
					...
					if (in_array($this_page, $aside_menu['validate'])) {
						...
						self::$app_controller->get_view   ('PropertyVenues', $pass);
						self::$app_controller->get_footer (array('page' => 'property_venues'));
						exit();
					}else{
	...
	public function post($request) {
		...
		switch ($subRequest) {
			...
			case 'SaveNewVenue':
				$prop_id 				= self::$app_controller->sanitise_string($request->parameters['prop_id']);
				$VenueName 				= self::$app_controller->sanitise_string($request->parameters['VenueName']);
				$DaysOpen 				= $request->parameters['DaysOpen'];
				$TimesFrom 				= $request->parameters['TimeFrom'];
				$TimesTo 				= $request->parameters['TimeTo'];
				$UploadVenueImageFile 	= $_FILES['UploadVenueImageFile'];
				$company_id 			= $_SESSION['company_id'];


				$save 					= self::add_new_venue ($company_id, $prop_id, $VenueName, $UploadVenueImageFile, $DaysOpen, $TimesFrom, $TimesTo);
	...
	static public function set_up_venues_table ($prop_id) {

		$return 	 	= array();
		$company_id 	= $_SESSION['company_id'];
		$table   	 	= self::$app_controller->get_all_prop_venues ($prop_id);

		foreach ($table as $c) {

			$button 	= '<button class="btn btn-info btn-xs " data-title="Edit" data-toggle="modal" data-target="#EditVenueModal" data-doc-id="'.$c['id'].'" aria-expanded="false"><span class="fa fa-pencil-square-o"></span></button>';


			$button 	.= '<button class="btn btn-danger btn-xs " data-title="Delete" data-toggle="modal" data-target="#DeleteVenueModal" data-doc-id="'.$c['id'].'" aria-expanded="false"><span class="fa fa-times"></span></button>';

			// '../companies/' . $company_id .'/properties/' . $prop_id;

			$img  	= 'http://connectliving.co.za/companies/'.$company_id . '/properties/' .$prop_id . '/' . $c['image'];
			$image 	= '<img src="'.$img.'" style="width:20%" />';
	...
	//add_new_venue ($company_id, $prop_id, $VenueName, $UploadVenueImageFile, $DaysOpen)
	static public function add_new_venue ($company_id, $prop_id, $VenueName, $UploadVenueImageFile, $DaysOpen, $TimesFrom, $TimesTo) {

		if (!self::$app_controller->validate_variables ($VenueName, 3)) {
			return array('status'  => false, 'text' => 'Invalid Venue Name');
		}

		if (!isset($UploadVenueImageFile)) {
			return array('status'  => false, 'text' => 'Please Select File');
		}

		if (!is_array($DaysOpen)) {
			return array('status'  => false, 'text' => 'Please Select Days Open');
		}

		// insert

		// die(var_dump($DaysOpen));

		$DaysOpenArray 	= implode(',', $DaysOpen);

		$temp 			= explode(".", $UploadVenueImageFile["name"]);
		$ext 			= end($temp);
		...
		$file_name  = self::$app_controller->upload_file ($UploadVenueImageFile, $dir);


		if ($file_name) {
			$venue_id 	= self::$app_controller->insert_new_venue (
										$prop_id,
										$file_name,
										$VenueName
									);

			// die(var_dump($venue_id));
			...
			if ($venue_id) {
				...
				foreach ($DaysOpen as $day) {
					...
					// die();
					if (!$TimesFrom[$day]) {
						$delete 		= self::$app_controller->delete_delete_venue ($venue_id);
						return array('status'  => false, 'text' => 'Please Time Open For ' .$day);
					}

					if (!$TimesTo[$day]) {
						$delete 		= self::$app_controller->delete_delete_venue ($venue_id);
						return array('status'  => false, 'text' => 'Please Time Close For ' .$day);
					}

					$timeopen  = $TimesFrom[$day];
					$timeclose = $TimesTo[$day];

					$save 	   = self::$app_controller->insert_new_venue_times (
												$venue_id,
												$day,
												$timeopen,
												$timeclose
					...
					if (!$save) {
						$delete 		= self::$app_controller->delete_delete_venue ($venue_id);
						return array('status'  => false, 'text' => 'Error inserting venue times for ' .$day. ', ' .$save.$delete);
					}

					$count ++;
				}
				return array('status'  => true, 'text' => 'Venue Added Successfully');
			}else{
