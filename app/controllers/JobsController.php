...
class JobsController
{
	...
	public function get($request) {
		...
		switch ($subRequest) {
			...
			case 'GetAllJobs':
				...
				$prop_name 		= self::$app_controller->sanitise_string($request->parameters['prop_name']);
				$company_id 	= $_SESSION['company_id'];
				$queries 		= self::set_up_jobs($prop_id, $prop_name, $company_id);
				return json_encode($queries);
				break;
			...
			case 'GetAllCards':
				...
				$company_id 	= $_SESSION['company_id'];
				$graph 			= self::set_up_cards ($prop_id, $company_id );
				...
				if (self::$app_controller->check_if_logged ($email)) {
					...
					if (in_array($this_page, $aside_menu['validate'])) {
						$pass 		= array(
										...
										'email' 	 => $email,
										'page_title' => 'Jobs ' .self::$property_name,
										'page'		 => $current,
						...
						self::$app_controller->get_view ('Jobs', $pass);
	...
	/*** save permissions ***/
	static public function set_up_jobs ($prop_id, $prop_name, $company_id) {
		$return_arr = array();
		$properties = self::$app_controller->get_job_list($prop_id);
	...
	static public function filter_cards ($prop_id, $company_id, $JobStatus) {
		...
		$queries 		= self::$app_controller->get_job_list_per_status ($prop_id, $JobStatus);
		$admin_user	  	= self::$app_controller->get_all_admin_managers ($company_id);
	...
	static public function set_up_cards($prop_id, $company_id) {
		$arry 			= array();

		$queries 		= self::$app_controller->get_job_list ($prop_id);
		$admin_user	  	= self::$app_controller->get_all_admin_managers ($company_id);
