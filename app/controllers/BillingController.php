...
class BillingController
{
	...
	public function get($request) {
		...
		switch ($subRequest) {
			case 'GetAllBilling':
				...
				$billing 	= self::set_up_billing ($prop_id, $prop_name);
			...
			case 'GetBillingInfo':
				...
				$get 		= self::$app_controller->get_billing_byid ($id);
			...
			case 'AssignUser': 
				...
				$assign 	= self::$app_controller->assign_billing_user ($ID, $AssineeID);
			...
			default:
				if (self::$app_controller->check_if_logged($email)) {
					...
					$current 		= 'billing' . self::$property_id;
					...
					if (in_array($this_page, $aside_menu['validate'])) {
						$pass 		= array(
										...
										'email' 	 => $email,
										'page_title' => 'Billing ' .self::$property_name,
										'page'		 => $current,
						...
						self::$app_controller->get_view ('Billing', $pass);
						self::$app_controller->get_footer (array('page'=>'billing'));
						exit();
					}else{
	...
	/*** save permissions ***/
	static public function set_up_billing ($prop_id, $prop_name) {
		$return_arr =  array();
		$properties = self::$app_controller->get_billing_list($prop_id);
	...
	static public function mark_done  ($Comment, $ID) {
		...
		$save 			= self::$app_controller->save_billing_comment ($Comment, $ID);
	...
	/*** save query ***/
	static public function submit_query ($QueryType, $UserID, $AssineeID, $Query, $Property, $Unit) {
		...
		$save 		= self::$app_controller->save_billing_query ($QueryType, $UserID, $AssineeID, $Query, $Property, $Unit);
	...
	static public function edit_query ($ID, $QueryType, $UserID, $Query, $Property, $Unit) {
		...
		$queryinfo 		= self::$app_controller->get_billing_byid ($ID);
		...
		$edit 		= self::$app_controller->edit_billing_query ($ID, $QueryType, $UserID, $Query, $Property, $Unit);
